...
static msg_t Update(void *arg){
	...
	while(TRUE){
		...
		mag_final[0] = mag_raw[0] * RANGE / 10;
		mag_final[1] = mag_raw[1] * RANGE / 10;
		mag_final[2] = mag_raw[2] * RANGE / 10;

		dcmUpdate(gyro_rates, ((float)acc_final[0])/1000, ((float)acc_final[1])/1000, ((float)acc_final[2])/1000, calc_gyro_rate(gyro[0], GYRO_XSENS), calc_gyro_rate(gyro[1], GYRO_YSENS), calc_gyro_rate(gyro[2], GYRO_ZSENS), (float)mag_final[0], (float)mag_final[1], (float)mag_final[2], interval);
